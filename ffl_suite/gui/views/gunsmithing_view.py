...
class GunsmithingView(ctk.CTkFrame):
    ...
    def create_widgets(self):
        ...
        self.board = ctk.CTkScrollableFrame(self, fg_color="transparent", orientation="horizontal")
        ...
        for status in statuses:
            col = ctk.CTkFrame(self.board, width=300, corner_radius=10)
            col.pack(side='left', fill='y', padx=10, pady=10)
            ctk.CTkLabel(col, text=status, font=ctk.CTkFont(size=16, weight="bold")).pack(pady=10)
    ...
    def create_job_card(self, parent, job):
        card = ctk.CTkFrame(parent, fg_color=("gray80", "gray30"))
        card.pack(fill='x', pady=5, padx=5)

        ctk.CTkLabel(card, text=f"#{job[0]} - {job[1]}", font=ctk.CTkFont(weight="bold")).pack(anchor='w', padx=5)
        ctk.CTkLabel(card, text=job[2]).pack(anchor='w', padx=5)
        ctk.CTkLabel(card, text=job[3], font=ctk.CTkFont(size=10)).pack(anchor='w', padx=5)
